(function() {
  const phantomConsole = console; //workaround due to console context issue with phantom2
  ...
  var errorHandler = function(msg, trace) {
    ...
    phantomConsole.error(msgStack.join('\n'));
    phantom.exit(1);
  };
  ...
  if (args.length !== 2) {
    phantomConsole.log("Need a url as the argument");
    phantom.exit(1);
  }
  ...
  page.onCallback = function(data) {
    if (data.event === 'exit') {
      ...
      phantomConsole.log('unknown event callback: ' + data);
    }
  };
  ...
  var address = args[1];
  phantomConsole.log('Running: ' + address);
  ...
  page.open(address, function(status) {
    if (status === "success") {
      ...
      phantomConsole.log("can't load the address!");
